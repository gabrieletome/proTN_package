...
read_proteomics <- function(software, folder, use_proteinGroups_MQ = FALSE, annotation_filename = "annotation",
  ...
  if(length(pep_filename) > 1){
    stop("Multiple files with same pattern")
  }
  if(software == "PD"){
  ...
  proteome_data = NULL
  if(software == "PD" & !(length(pep_filename)==0)){
  ...
  } else if(software == "PD" & length(pep_filename)==0){
    message("Reading Proteome Discover files based on Proteins only...")
    proteome_data = read_PD_prot_only_files(anno_filename, prot_filename, 
                                  condition_col=condition_col, 
                                  sample_col=sample_col, 
                                  color_col=color_col,
                                  batch_corr_exe= batch_corr_exe, 
    ...
    proteome_data = read_MQ_files(anno_filename, pep_filename, 
                                  ...
                                  batch_col = batch_col,
                                  min_peptide_protein = min_peptide_protein)
  } else if(software == "MQ" & use_proteinGroups_MQ & !(length(pep_filename)==0)){
    ...
    message("Reading MaxQuant files (proteinGroups.txt)...")
    proteome_data = read_MQ_prot_files(anno_filename, prot_filename, 
                                               condition_col=condition_col, 
                                               sample_col=sample_col, 
                                               color_col=color_col,
                                               batch_corr_exe = batch_corr_exe, 
...
# Read Proteome Discoverer
read_PD_prot_only_files <- function(anno_filename, prot_filename,
                          condition_col="Condition", sample_col="Sample", 
                          color_col="Color", batch_corr_exe = FALSE, batch_col="batch",
                          filt_absent_value = 0, min_peptide_protein=0){
  input_files <- list()
  ...
  abundance_cols <- grep("Abundance", names(input_files$PROT), value = TRUE)
  input_files$PROT <- input_files$PROT[, c("Accession","Description", "Modifications", "# Peptides", abundance_cols), with = FALSE]
  ...
  message(paste0("\tProtein removed due to missing Gene Names: ",to_remove))
  # Merge PROT and PEP tables
  input_files$PD_PEP_matrix <- input_files$PROT
  
  # Filter sample in file annotation
  colToKeep <- c(c("Accession","Description","GeneName","Modifications", "# Peptides"),
                 lapply(input_files$annotation$File_ID, function(x){
                   colnames(input_files$PD_PEP_matrix)[grepl(str_c(x,":"), colnames(input_files$PD_PEP_matrix))]
                 }) %>% unlist())
  input_files$PD_PEP_matrix<-input_files$PD_PEP_matrix[, ..colToKeep]
  ...
  colnames(input_files$PD_PEP_matrix)<-c(c("Accession","Description","GeneName","Modifications","N_peptides"),
                                         unlist(lapply(colnames(input_files$PD_PEP_matrix), function(x){
                                           lapply(input_files$annotation$File_ID, function(y){if(str_detect(x,paste0(y,":"))){
                                             input_files$annotation$Sample[which(input_files$annotation$File_ID %in% y)]
                                           }})
  ...
  #Maintain only the new complete matrix of abundance
  input_files$PD_PEP_matrix[, ID_peptide := as.factor(paste(input_files[["PD_PEP_matrix"]]$GeneName, input_files[["PD_PEP_matrix"]]$Modifications, sep="_"))]
  ...
  #Made the description of the mpeptides
  psm_peptide_table <- as.data.table(unique(input_files[["PD_PEP_matrix"]][, c("ID_peptide","Accession","Description","GeneName","Modifications","N_peptides")]))
  ...
  psm_anno_raw <- data.table("Accession" = psm_peptide_table$Accession,
                             ...
                             "symbol"=psm_peptide_table$GeneName,
                             "modifications"=psm_peptide_table$Modifications,
                             "N_peptide" = psm_peptide_table$N_peptides)
  ...
  psm_log_prot_df <- psm_log_prot_df[ID_peptide %in% filter_df_single_pep]
  psm_anno_df <- psm_anno_df[ID_peptide %in% filter_df_single_pep]
  psm_anno_df <- psm_anno_df[!duplicated(psm_anno_df$symbol)]
  psm_peptide_table <- as.data.table(psm_peptide_table)[ID_peptide %in% filter_ID_peptide]
  psm_peptide_table <- psm_peptide_table[!duplicated(psm_peptide_table$GeneName)]
...
# Read MaxQuant files
read_MQ_prot_files <- function(anno_filename, prot_filename,
                                       condition_col="Condition", sample_col="Sample", 
                                       color_col="Color", batch_corr_exe = FALSE, batch_col="batch",
                                       filt_absent_value = 0, min_peptide_protein = 0){
  input_files <- list()
  ...
  # Check if "Gene Names" and "Protein names" columnare present. Otherwise add them
  # TODO: check if true for all cases
  if(!("Gene names" %in% colnames(input_files[["PROT"]]))){
    input_files[["PROT"]][, `Gene names` := tstrsplit(tstrsplit(`Majority protein IDs`, "\\|", keep = 3)[[1]], "_", keep = 1)[[1]]]
    input_files[["PROT"]][, `Majority protein IDs` := tstrsplit(`Majority protein IDs`, "\\|", keep = 2)[[1]]]
  }
  if(!("Protein names" %in% colnames(input_files[["PROT"]]))){
    data("anno_uniprot", envir = environment())
    anno_uniprot[, `Gene Names`:=NULL]
    if(length(intersect(anno_uniprot$`Majority protein IDs`, input_files[["PROT"]]$`Majority protein IDs`)) == 0){
      input_files[["PROT"]][, `Protein names` := `Majority protein IDs`]
    }else{
      input_files[["PROT"]] <- merge.data.table(input_files[["PROT"]], unique(anno_uniprot, by="Majority protein IDs", fromLast=FALSE), 
                                               by="Majority protein IDs", 
                                               all.x = TRUE)
    }
  } 
  if(nrow(input_files[["PROT"]][is.na(`Protein names`)])>0){
    data("anno_uniprot", envir = environment())
    anno_uniprot[, `Gene Names`:=NULL]
    unique_anno_uniprot <- unique(anno_uniprot, by="Majority protein IDs", fromLast=FALSE)
    setnames(unique_anno_uniprot, old=c("Protein names"), new = c("desc"))
    input_files[["PROT"]][, `Protein names` := as.character(`Protein names`)]
    input_files[["PROT"]][is.na(`Protein names`), `Protein names` := unique_anno_uniprot[.SD, on=.(`Majority protein IDs`), x.desc]]
  }
  ...
  input_files[["PROT"]][, Modifications:=NA]
  ...
  input_files[["PROT_PEP"]] <- input_files[["PROT"]]
  suppressWarnings({
    input_files[["PROT_PEP"]] <- melt(input_files[["PROT_PEP"]], 
                                      id.vars = c("Leading razor protein", "Protein names", "Fasta headers", "Gene names", "Unique peptides"),
                                      measure.vars = grep("Intensity ", names(input_files$PROT), value = TRUE), 
                                      variable.name = "Raw file", 
                                      value.name = "Intensity")
  })
  input_files[["PROT_PEP"]][, `Raw file` := stri_replace(`Raw file`, regex = "Intensity ", replacement = "")]
  ...
  #Made the matrix
  psm_sig_raw <- data.table("ID_peptide" = as.factor(input_files[["PROT_PEP"]]$`Gene names`), 
                            "Sample" = as.factor(input_files[["PROT_PEP"]]$`Raw file`), 
                            "Intensity" = input_files[["PROT_PEP"]]$Intensity)
  suppressWarnings({
    psm_sig_raw <- psm_sig_raw[, .(Intensity = sum(Intensity)), by = c("ID_peptide", "Sample")]
  ...
  #Made the description of the mpeptides
  psm_peptide_table <- as.data.table(unique(input_files[["PROT_PEP"]][, c("Leading razor protein", "Protein names", "Fasta headers", "Gene names", "Unique peptides")]))
  colnames(psm_peptide_table) <- c("Accession","Description","FASTA_description","GeneName","N_peptide")
  psm_peptide_table[, ID_peptide := GeneName][N_peptide == 0, N_peptide := 1]
  ...
  psm_anno_raw <- data.table("Accession" = psm_peptide_table$Accession,
                             "ID_peptide"=psm_peptide_table$ID_peptide,
                             "symbol"=psm_peptide_table$GeneName,
                             "N_peptide"=psm_peptide_table$N_peptide)
  ...
  psm_anno_df <- psm_anno_df[ID_peptide %in% filter_df_single_pep]
  psm_anno_df <- psm_anno_df[!duplicated(psm_anno_df$symbol)]
  psm_peptide_table <- as.data.table(psm_peptide_table)[ID_peptide %in% filter_df_single_pep]
  psm_peptide_table <- psm_peptide_table[!duplicated(psm_peptide_table$GeneName)]
